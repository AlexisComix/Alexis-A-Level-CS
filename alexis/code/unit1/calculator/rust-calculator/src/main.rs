...
fn main() {
    ...
    let first_operand: f64 = get_operand();
    let second_operand: f64 = get_operand();

    let result: f64 = do_operation(operatornum, first_operand, second_operand);
...
fn get_operand() -> f64 {
    ...
    let num: f64 = input.trim().parse().expect("Not a valid number");
    
    return num;
} 

fn do_operation(operation: u8, operand1: f64, operand2: f64) -> f64 {
